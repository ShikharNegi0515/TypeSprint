...
export class AchievementsService {
  ...
  async checkAndAwardAchievements(userId: string, stats: { wpm: number, accuracy: number, duration: number, totalTests: number }) {
    ...
    if (stats.totalTests >= 1 && !unlockedIds.has('first_test')) newlyUnlocked.push('first_test');
    if (stats.wpm >= 100 && !unlockedIds.has('speed_demon')) newlyUnlocked.push('speed_demon');
    if (stats.accuracy >= 100 && !unlockedIds.has('perfectionist')) newlyUnlocked.push('perfectionist');
    if (stats.duration >= 120 && !unlockedIds.has('marathon')) newlyUnlocked.push('marathon');
